...
def is_stage_2b3_default_layout(layout):
	try:
		current = json.loads(layout or "[]") if isinstance(layout, str) else layout
		stage_2b3_default = json.loads(stage_2b3_manager_dashboard_layout())
	except (TypeError, json.JSONDecodeError):
		return False
	return current == stage_2b3_default


def get_updated_layout(layout):
	if not is_stage_2b3_default_layout(layout):
		return None
